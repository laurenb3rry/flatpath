...
start rather than route on nonsense.
    ...
    header
        ...
        edge_count   uint32
        name_count   uint32
        cost_count   uint32   how many hill-aversion settings each edge carries
    ...
    edges            edge_count records, sorted by origin node
        ...
        costs        cost_count x float32   routing cost per setting
...
dominate the file size and make edge records variable-width. And elevation change
and plain walking time are stored per edge even though both could be recomputed
on the fly, because the route cards need them for every edge of a chosen route
and recomputing means shipping the cost function to the app.
"""
...
def _edge_dtype(cost_count):
    ...
    fields += [(f"cost{c}", "<f4") for c in range(cost_count)]
    fields += [("name_index", "<u4")]
...
def write(path, lats, lons, elevations, edges, names):
    ...
    cost_count = int(edges["costs"].shape[0])
    ...
    edge_records = np.empty(edge_count, dtype=_edge_dtype(cost_count))
    for field in ("from_node", "to_node", "length_m", "delta_elev_m", "time_s", "name_index"):
        edge_records[field] = edges[field]
    for c in range(cost_count):
        edge_records[f"cost{c}"] = edges["costs"][c]

    with open(path, "wb") as out:
        out.write(_HEADER.pack(
            MAGIC, FORMAT_VERSION, node_count, edge_count, len(names), cost_count
        ))
...
def read_header(path):
    ...
    with open(path, "rb") as source:
        magic, version, node_count, edge_count, name_count, cost_count = _HEADER.unpack(
